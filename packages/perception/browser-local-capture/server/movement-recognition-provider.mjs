...
export function buildVisualConversationPrompt(input = {}) {
  ...
  return [
    ...
    "When previous context contains spatial_facts, treat high-confidence facts as grounded evidence.",
    "Do not invent spatial relations or exact metric distances that are absent from spatial_facts.",
    "If spatial precision is unavailable, answer with ordinary visual reasoning and briefly disclose that limitation.",
    "Use partial observations and one suggested view when the spatial facts provide them.",
    "Do not proactively narrate unrelated movement.",
    "Do not identify the person or infer sensitive attributes.",
    "If visual evidence is uncertain, describe only what is supported, disclose uncertainty, and ask one useful clarifying question.",
    "",
    ...
    "",
    "Rules:",
    "- default to 1-3 conversational sentences and about 15-70 spoken words",
    ...
    "- if unsure, set uncertainty true and give a Conversation answer about the current view; never use movement-failure or what-changed language",
    "- return concise JSON only",
    "- return confidence conservatively"
  ].join("\n");
}
